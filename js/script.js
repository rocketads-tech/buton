document.addEventListener("DOMContentLoaded", () => {
  ...
  if (mainSliderEl && thumbsSliderEl && typeof Swiper !== "undefined") {
    const thumbsSlider = new Swiper(".product-slider-thumbs", {
      ...
      freeMode: true, // позволяет свободно тянуть
      touchRatio: 1,
      touchAngle: 45,
    });
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const closeButtons = [backdrop, closeBtn]; // клик по оверлею закрывает

	// Функция открытия
	const openMenu = () => {
		body.classList.add('mobile-menu--open');
		menu.setAttribute('aria-hidden', 'false');
		// Опционально: запретить скролл фона
...
// ===== COUNTER =====
// Найдём все счётчики на странице
